...
void BindingHandler::OnInvokeCommandFailure(DeviceProxy * aDevice, BindingData & aBindingData, CHIP_ERROR aError)
{
    ...
    {
        PLAT_LOG("Response timeout for invoked command, trying to recover CASE session.");
        if (!aDevice)
            return;
        ...
        {
            PLAT_LOG("Disconnecting from CASE session failed due to: %" CHIP_ERROR_FORMAT, error.Format());
            return;
        }
    ...
    {
        PLAT_LOG("Binding command was not applied! Reason: %" CHIP_ERROR_FORMAT, aError.Format());
    }
}
                                         ...
                                         void * aContext)
{
    ...
    auto onSuccess = [](const ConcreteCommandPath & commandPath, const StatusIB & status, const auto & dataResponse) {
        PLAT_LOG("Binding command applied successfully!");
    ...
    case Clusters::OnOff::Commands::Off::Id:
        ...
        break;
    default:
        PLAT_LOG("Invalid binding command data - commandId is not supported");
        break;
    }
    if (CHIP_NO_ERROR != ret)
    {
        PLAT_LOG("Invoke OnOff Command Request ERROR: %s", ErrorStr(ret));
    }
}
                                                ...
                                                DeviceProxy * aDevice, void * aContext)
{
    ...
    if (CHIP_NO_ERROR != ret)
    {
        PLAT_LOG("Invoke Group Command Request ERROR: %s", ErrorStr(ret));
    }
}

void BindingHandler::LightSwitchChangedHandler(const EmberBindingTableEntry & binding, DeviceProxy * deviceProxy, void * context)
{
    VerifyOrReturn(context != nullptr, PLAT_LOG("Invalid context for Light switch handler"););
...
{
    VerifyOrReturn(context != nullptr, PLAT_LOG("Invalid context for Light switch context release handler"););
...
{
    PLAT_LOG("Initialize binding Handler");
    auto & server = Server::GetInstance();
    if (CHIP_NO_ERROR !=
        BindingManager::GetInstance().Init(
            { &server.GetFabricTable(), server.GetCASESessionManager(), &server.GetPersistentStorage() }))
    {
        PLAT_LOG("BindingHandler::InitInternal failed");
    }
...
void BindingHandler::PrintBindingTable()
{
    ...
    PLAT_LOG("Binding Table size: [%d]:", bindingTable.Size());
    uint8_t i = 0;
    for (auto & entry : bindingTable)
    {
        switch (entry.type)
...
{
    VerifyOrReturn(aContext != 0, PLAT_LOG("Invalid Swich data"));

    BindingData * data = reinterpret_cast<BindingData *>(aContext);
    PLAT_LOG("Notify Bounded Cluster | endpoint: %d cluster: %d", data->EndpointId, data->ClusterId);
